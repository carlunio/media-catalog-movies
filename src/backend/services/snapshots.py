...
def _update_import_state(
    snapshot: dict[str, Any], backup_path: Path | None
) -> dict[str, Any]:
    state = _read_json(SYNC_STATE_PATH) or {}
    now = _iso_now()
    state.update(
...
def _find_snapshot(snapshot_id: str) -> dict[str, Any]:
    ...
    for snapshot in list_snapshots(verify_hash=True, include_invalid=True):
        ...
        if not snapshot.get("valid"):
            error = str(snapshot.get("error") or "Snapshot no válido.")
...
def _backup_local_database(snapshot_id: str) -> Path | None:
    ...
    backup_path = _unique_path(
        backup_dir
        / f"{DB_PATH.stem}_before_import_{timestamp}_{snapshot_slug}{DB_PATH.suffix}"
    )
...
def import_snapshot(*, snapshot_id: str, confirm: bool = False) -> dict[str, Any]:
    if not confirm:
        raise SnapshotError(
            "Importar un snapshot sustituye la base local. Repite la operación con confirm=true."
        )
...
def cleanup_snapshots() -> dict[str, Any]:
    ...
    for snapshot in snapshots:
        key = (
            str(snapshot.get("source_actor") or ""),
            str(snapshot.get("source_device") or ""),
        )
